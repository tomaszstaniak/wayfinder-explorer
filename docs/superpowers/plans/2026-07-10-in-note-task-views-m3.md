...
- **Active-note tracking:** the view resolves the current note as `getActiveViewOfType(MarkdownView)?.file ?? workspace.getActiveFile()`.
- **Extraction source:** if the active Markdown view's file is the resolved file, extract from `editor.getValue()` (unsaved edits included); else `await vault.cachedRead(file)`.
...
- **Toggle wrapper:** `toggleTaskStatus(env, task)` — editor path uses `editor.getLine`/`replaceRange`; disk path uses `vault.process` + `applyStatusToLine`; both verify `task.raw` first.
- **Jump wrapper:** `leaf.openFile(file, { eState: { line: task.line } })` on the active tab.
...
In `DEFAULT_SETTINGS` (after `showTaskCounts: false,`):
...
export class WayfinderTasksView extends ItemView {
	...
	/** Re-read the active note and re-render its tasks. */
	async refresh(): Promise<void> {
		const { file, text } = this.activeSource();
		if (!file) {
			this.listEl.empty();
		...
		if (tasks.length === 0) {
			this.listEl.empty();
			this.listEl.createDiv({ cls: 'wayfinder-task-empty', text: 'No tasks in this note.' });
			return;
		}
	...
	private activeSource(): { file: TFile | null; text: string } {
		const md = this.plugin.app.workspace.getActiveViewOfType(MarkdownView);
		const file = md?.file ?? this.plugin.app.workspace.getActiveFile();
		if (!file) return { file: null, text: '' };
		if (md?.file === file) return { file, text: md.editor.getValue() };
		return { file, text: '' }; // async disk read happens in refreshAsync path (Step 4 note)
	}

	private async toggle(file: TFile, task: import('./task-extract').ExtractedTask): Promise<void> {
		const md = this.plugin.app.workspace.getActiveViewOfType(MarkdownView);
		const editor = md && md.file?.path === file.path ? md.editor : null;
		const env: ToggleEnv = {
			editor: editor
				? {
						getLine: (line) =>
...
- [ ] **Step 4: Make extraction read disk when needed**

The `activeSource()` in Step 2 returns `text: ''` when the file is not the active editor (it cannot `await` synchronously). Replace `refresh()` + `activeSource()` with an async source that reads `cachedRead` when there is no live buffer:

Replace the `activeSource` method with:
...
And update the first line of `refresh()` to await it:

```ts
		const { file, text } = await this.activeSource();
```

- [ ] **Step 5: Add the setting toggle + minimal styles**
...
- [ ] **Step 7: Manual verification (Obsidian)**

Reload the plugin (toggle off/on in Community plugins), then verify:
...
4. Click a task's **checkbox** → the note's line flips (`- [ ]` ↔ `- [x]`) and the pane regroups.
5. Click a task's **text** → the editor scrolls/moves to that line.
6. Settings → Wayfinder → **Tasks sidebar** off → ribbon disappears and the pane closes; the open command shows a Notice. Turn on → ribbon returns.
...
Expected: PASS. If it hard-codes a settings literal missing `showTaskSidebar`, add `showTaskSidebar: true` to that literal so it matches `DEFAULT_SETTINGS`.
...
- Editor-buffer-first extraction, `cachedRead` fallback → Task 2 Step 4. ✅
- Grouped render via M2 renderer → Task 2 (`renderTaskList`). ✅
- Toggle (editor-first, disk fallback, non-fuzzy) → Task 1 (`toggleTaskStatus`, unit-tested) + Task 2 env wiring. ✅
- Jump to line → Task 2 (`jump`). ✅
- Register at load; setting gates ribbon/render → Task 2 (`registerView` + `syncTasksSidebar`). ✅
- Debounced refresh on workspace/vault/metadata events → Task 2. ✅

**Placeholder scan:** none — Task 2 Step 4 explicitly supersedes the Step 2 synchronous stub (called out, not left dangling).

**Type consistency:** `ToggleEnv`/`EditorLineIO`/`DiskIO` match between `task-actions.ts` and the view's env construction. `VIEW_TYPE_TASKS`, `WayfinderTasksView`, `showTaskSidebar` names are consistent across files. `renderTaskList` handler shape matches M2.
